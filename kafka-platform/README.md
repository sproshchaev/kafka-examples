...
**Внешний адрес:** `localhost:8081` или `localhost:8085`
**Рекомендуется стандартизировать на:** `localhost:8081`

### 3. REST API (Kafka REST Proxy)
**Внешний адрес:** `localhost:8082` (во всех конфигурациях где присутствует)
...
2. **Для Schema Registry** везде использовать порт `8081` вместо `8085`
...
# В Schema Registry изменить порт:
ports:
  - "8081:8081"
environment:
